...
$(document).ready(function(){
	...
	if(user){
		login.innerHTML=`Hi~[<a href="#">退出登录</a>]`;
	}else{
		login.innerHTML=`Hi~[<a href="#">a请登陆</a>] [<a href="#">免费注册</a>]`;
	}
	console.log(user);
	//显示购物车
	var shop=$id("shop");
	...
	shop.onclick=function(){
		...
		if(user){
			...
			if(shopCar){
				...
				shopBox.innerHTML=`<div class="iconfont" style="font-size:200px;line-height:350px">&#xe63b;</div>购物车空空如也!`;
			}
		}else{
			shopBox.style.textAlign="center";
	...
	var mainFoot=$(".main-foot ").children("[class=main-box]")[0];
	 $.get("./json/maintop.json",function(data){
       ...
       for (var i = 0; i < data.length; i++) {
       	str+=`<a href="#${data[i].id}"><img src=${data[i].src}></a>`;
       }
...
box.onmousemove=function(){
	clearInterval(timr);
	animate(btnarr,{opacity:100});
}
box.onmouseout=function(){
	 timr=setInterval(showImg,5000);
	 animate(btnarr,{opacity:0});
}
...
function lb(){
	for (var i = 0; i <boxarr.length-1; i++) {
        animate(boxarr[i],{opacity:0},null,30);
    }	
	 	animate(boxarr[index],{opacity:100},null,30);
	 	for (var i = 0; i < btns.length; i++) {
...
//轮播图2
var trbox=$id("trademarkBox");trLeft
